...
|----|-------------|--------|---------|------|--------|-------|
| POLISH-01 | "Explorar Jogos" appears once in desktop nav | 🔵 AUTO — PASS | | | | `nav.html`: one `.nav-cta[data-href="explorar/explorar.html"]` in desktop area; one in overlay. Desktop shows only the first. |
| POLISH-02 | Nav consistent (no colored bar) on all pages | | | | | Visual — browser check required |
| POLISH-03 | Game cards: 3-col grid + 1 "Em Breve" placeholder | 🔵 AUTO — PASS (structure) | | | | `cards.css`: `.game-grid { grid-template-columns: repeat(3, 1fr) }`; `games.js` renders placeholder card. Visual alignment requires browser. |
...
| POLISH-06 | Explore filter bar has no opaque blue strip | | | | | Requires visual check on explore page |
| POLISH-07 | Background vertical hierarchy visible | | | | | Visual — needs browser gradient inspection |
...
| ID | Requirement | Chrome | Firefox | Edge | Mobile | Notes |
|----|-------------|--------|---------|------|--------|-------|
| CAROUSEL-01 | CSS 3D orbit ring visible, tilted ellipse | | | | | Visual — browser check |
...
| CAROUSEL-02 | 5 planets at 72° intervals; back planets blurred | 🔵 AUTO — PASS (logic) | | | | `carousel-3d.js`: `STEP_DEG = 72`, `PLANET_COUNT = 5`; zone "back" → `filter: blur(var(--orbita-blur-fundo))` in CSS |
...
| CAROUSEL-06 | Side planets ~65% apparent size via perspective | | | | | Visual — requires 3D rendering in browser |
| CAROUSEL-07 | Keyboard nav (← → Home End Enter) | | | | | Keyboard check needed in browser |
...
| CAROUSEL-07 | `aria-live` announcer fires on navigation | | | | | Requires screen reader or AT inspection |
...
| ID | Requirement | Chrome | Firefox | Edge | Mobile | Notes |
|----|-------------|--------|---------|------|--------|-------|
| ANIM-01 | Starfield drifts continuously, no seam | | | | | Visual — browser check |
...
| ANIM-02 | Planet spheres float vertically | | | | | Visual — browser check |
...
| ANIM-03 | Sparkle accents twinkling | | | | | Visual — browser check |
...
| ANIM-05 | Tab hidden: animations pause | 🔵 AUTO — PASS (code) | | | | `animacoes.js`: `visibilitychange` listener toggles `.tab-hidden` on body; `animacoes.css`: `.tab-hidden * { animation-play-state: paused !important }` |
...
| ID | Requirement | Chrome | Firefox | Edge | Mobile | Notes |
|----|-------------|--------|---------|------|--------|-------|
| TRANS-01 | Footer wave divider separates sections | | | | | Visual — browser check |
...
| TRANS-02 | Sub-pixel gap visual check at HiDPI | | | | | Requires browser at 125%+ zoom |
...
| SHELL-02 | `sandbox="allow-scripts allow-same-origin"` | 🔵 AUTO — PASS | | | | `jogar.html`: `<iframe sandbox="allow-scripts allow-same-origin" allow="autoplay" title="Área do jogo">` confirmed |
| SHELL-03 | Loading overlay shows until game-ready or 5s timeout | | | | | Requires browser runtime check |
